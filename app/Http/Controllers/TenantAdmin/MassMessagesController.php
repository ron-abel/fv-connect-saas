...
class MassMessagesController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $mass_messages = MassMessage::select('mass_messages.*', 'mass_messages.created_at', 'users.full_name as created_by')->leftJoin('users', 'mass_messages.created_by', '=', 'users.id')->where('mass_messages.tenant_id', $this->cur_tenant_id)->with('mass_message_logs')->orderBy('mass_messages.id', 'desc')->get();

            $variable_keys = Variable::getVariableKeyByPage('is_mass_text');

            return $this->_loadContent("admin.pages.mass_messages", [
     ...
     */
    public function mass_messages_logs($domain, $id)
    {
        ...
        $mass_message_logs = DB::table('mass_messages')
            ...
            ->where('mass_messages.tenant_id', '=', $this->cur_tenant_id)
            ->where('mass_message_logs.mass_message_id', '=', $id)
            ->get();
    ...
    public function fetch_contacts(Request $request)
    {
        $params = [
            ...
            'requestedFields' => "personId,phones,fullName"
        ];
    ...
    private function getContactsByPersonType($params, $is_exclude_blacklist)
    {
        try {
            ...
            $contacts = collect($contacts)->where('phones', '!=', null)->toArray();
            $contacts_data = [];

            foreach ($contacts as $contact) {
                if (isset($contact->phones)) {
                    foreach ($contact->phones as $phone) {
                        if (isset($phone->rawNumber) && !empty($phone->rawNumber)) {
                            array_push($contacts_data, [
                                'person_name' => $contact->fullName,
                                'person_number' => $phone->rawNumber
                            ]);
                        }
                    }
                }
            ...
            return [
                ...
                "note" => $num_of_contact . " contacts, " . count($contacts_data) . " numbers"
            ];
        } catch (\Exception $ex) {
    ...
    public function upload_csv(Request $request)
    {
        try {
            ...
            $target_dir =  public_path('/assets/uploads/mass-messages');
            ...
            if ($index > 0) {
                if (isset($csv_content[1]) && !empty($csv_content[1])) {
                    array_push($contacts, [
                        ...
                        'person_number' => preg_replace('/[^0-9]+/', '', $csv_content[1]),
                    ]);
                }
            }
        }
    ...
    public function save_messages_and_dispatch($data, $mass_message_data)
    {
        try {
            ...
            $data['is_schedule_job'] = (isset($data['is_schedule_job']) && $data['is_schedule_job']) ? true : false;
            ...
            $mass_message = MassMessage::create($data);

            $mass_message_logs = $mass_message->mass_message_logs()->createMany($mass_message_data);
    ...
    /**
     * Get Mass Message Log In CSV Format
     */
    public function massMessagesExportcsv($domain, $id)
    {
        $service = new ExportService();
        return $service->exportMassMessagesCSV($id);
    }

    /**
     * [GET] Mass Message Custom Log
     */
    public function massMessagesCustomLog(Request $request)
    {
    ...
    /**
     * [POST] Generate Mass Message Log from mass_message_logs
     */
    public function massMessagesCustomLogCSV(Request $request)
    {
        $log_start_date = $request->log_start_date;
        $log_end_date = $request->log_end_date;

        $service = new ExportService();
        return $service->exportMassMessagesCustomCSV($this->cur_tenant_id, $log_start_date, $log_end_date);
    }

    /**
     * [POST] Delete Jobs, mass_message_logs & mass_messages
     */
    public function massMessagesDelete(Request $request)
    {
        $mass_messages_id = $request->input('mass_messages_id');
        DB::table('jobs')
            ->join('mass_message_logs', 'mass_message_logs.job_id', '=', 'jobs.id')
            ->where('mass_message_logs.mass_message_id', '=', $mass_messages_id)
            ->delete();
        MassMessageLog::where('mass_message_id', $mass_messages_id)->delete();
        MassMessage::where('id', $mass_messages_id)->delete();
    ...
    public function reCreateJob(Request $request)
    {
        try {
            ...
            if ($re_create_all_job) {
                ...
                $mass_message_logs = MassMessageLog::whereHas('mass_message', function ($query) {
                    $query->where('tenant_id', $this->cur_tenant_id);
                })->where('mass_message_id', $mass_messages_id)->get();
                ...
                $mass_message = MassMessage::find($mass_messages_id);
    ...
    /**
     *  [POST] Update mass message
     */
    public function updateMassMessages(Request $request)
    {
        try {
            $mass_messages_id = $request->input('mass_messages_id');
            $mass_message = MassMessage::find($mass_messages_id);
            $mass_message->message_body = $request->input('update_message_body');
            $mass_message->save();

            $job_delay_time = (int)env('MASS_TEXT_BUFFER_MINS', 10) * 60;

            if (!empty($mass_message->schedule_time)) {
                $schedule_time = date('Y-m-d H:i:s', strtotime($mass_message->schedule_time));
                $time_diff = strtotime($schedule_time) - time();
                if ($time_diff > 0) {
                    $job_delay_time = $time_diff;
                }
            }

            Logging::warning(json_encode($mass_message));
            Logging::warning($job_delay_time);
            ...
            return redirect(url("/admin/mass_messages"))->with('success', "Mass messages body updated successfully!");
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    }
